...
/                   Changed fname member in the FileInfo structure on Unicode cfg.
...
#if _VOLUMES
	static FileSystem *FatFs[_VOLUMES];	/* Pointer to the file system objects (logical drives) */
#else
...
static
int lock_fs (
	FileSystem *fs		/* File system object */
)
{
...
FRESULT chk_lock (	/* Check if the file can be accessed */
	Directory* dj,		/* Directory object pointing the file to be checked */
...
UINT inc_lock (	/* Increment file open counter and returns its index (0:int error) */
	Directory* dj,	/* Directory object pointing the file to register or increment */
	...
	if (clst < 2 || clst >= fs->n_fatent) {	/* Check range */
		...
		while (clst < fs->n_fatent) {			/* Not a last link? */
			...
			if (ecl + 1 == nxt) {	/* Is next cluster contiguous? */
				...
				disk_ioctl(CTRL_ERASE_SECTOR, rt);		/* Erase the block */
				scl = ecl = nxt;
			}
#endif
...
static
DWORD ld_clust (
	FileSystem *fs,	/* Pointer to the fs object */
...
static
FRESULT dir_find (
	Directory *dj			/* Pointer to the directory object linked to the file name */
)
{
	FRESULT res;
	BYTE c, *dir;
...
FRESULT dir_register (	/* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision, FR_DISK_ERR:Disk error */
	Directory *dj				/* Target directory with object name to be created */
)
{
	FRESULT res;
#if _USE_LFN	/* LFN configuration */
...
FRESULT dir_remove (	/* FR_OK: Successful, FR_DISK_ERR: A disk error */
	Directory *dj				/* Directory object pointing the entry to be removed */
)
{
	FRESULT res;
#if _USE_LFN	/* LFN configuration */
...
)
{
	...
	if (dj->sect) {
		...
		fno->Attribute = dir[DIR_Attr];				/* Attribute */
		fno->Size = LD_DWORD(dir+DIR_FileSize);	/* Size */
		fno->Date = LD_WORD(dir+DIR_WrtDate);		/* Date */
		fno->Time = LD_WORD(dir+DIR_WrtTime);		/* Time */
	}
	*p = 0;		/* Terminate SFN str by a \0 */

#if _USE_LFN
	if (fno->LName && fno->LSize) {
		TCHAR *tp = fno->LName;
		WCHAR w, *lfn;
		...
		if (dj->sect && dj->lfn_idx != 0xFFFF) {/* Get LFN if available */
			...
			while ((w = *lfn++) != 0) {			/* Get an LFN char */
				...
				if (_DF1S && w >= 0x100)		/* Put 1st byte if it is a DBC (always false on SBCS cfg) */
					tp[i++] = (TCHAR)(w >> 8);
#endif
				if (i >= fno->LSize - 1) { i = 0; break; }	/* Buffer overflow, no LFN */
				tp[i++] = (TCHAR)w;
			}
		}
...
FRESULT follow_path (	/* FR_OK(0): successful, !=0: error code */
	Directory *dj,			/* Directory object to return last directory and found object */
...
FRESULT chk_mounted (	/* FR_OK(0): successful, !=0: any error occurred */
	...
	FileSystem **rfs,		/* Pointer to pointer to the found file system object */
...
)
{
	...
	*rfs = fs;							/* Return pointer to the corresponding file system object */
	...
	sysect = nrsv + fasize + fs->n_rootdir / (SS(fs) / SZ_DIR);	/* RSV+FAT+Directory */
...
FRESULT validate (	/* FR_OK(0): The object is valid, !=0: Invalid */
	void* obj		/* Pointer to the object File/Directory to check validity */
)
{
	File *fil = (File*)obj;	/* Assuming offset of fs and id in the File/Directory is identical */
...
FRESULT f_mount (
	...
	FileSystem *fs		/* Pointer to new file system object (NULL for unmount)*/
)
{
	FileSystem *rfs;
	...
	if (res == FR_OK) {
		...
		if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
			...
			if (res != FR_OK) {					/* No file, create new */
				...
				if (dir[DIR_Attr] & (AM_RDO | AM_DIR)) {	/* Cannot overwrite it (R/O or Directory) */
					res = FR_DENIED;
				} else {
...
FRESULT f_readdir (
	Directory *dj,			/* Pointer to the open directory object */
...
FRESULT f_getfree (
	...
	FileSystem **fatfs		/* Pointer to return pointer to corresponding file system object */
)
{
	FRESULT res;
	FileSystem *fs;
		...
		if (res == FR_OK) {					/* The object is accessible */
			...
			if (res == FR_OK && (dir[DIR_Attr] & AM_DIR)) {	/* Is it a sub-dir? */
				if (dclst < 2) {
					...
					mem_cpy(&sdj, &dj, sizeof (Directory));	/* Check if the sub-dir is empty or not */
					sdj.sclust = dclst;
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {					/* Root directory */
				...
				ST_WORD(dir+DIR_WrtTime, fno->Time);
				ST_WORD(dir+DIR_WrtDate, fno->Date);
				dj.fs->wflag = 1;
				res = sync_fs(dj.fs);
			}
		}
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {						/* Old object is found */
			if (!djo.dir) {						/* Is root dir? */
				...
				mem_cpy(&djn, &djo, sizeof (Directory));		/* Check new object */
